      ...
      'bodyPlaceholder': `请输入请求体，使用JSON Schema格式。例如：
{
    ...
    "required": ["name", "age"]
        ...
        'clickOrDropCharacterCards': '点击或拖拽JSON格式的角色卡至此处',
        'noResults': '暂无结果',
        ...
        'TTSVoice': '音色',
        'TTSRate': '语速',
        'addTTSNotice': '添加供应商后，请返回此页面并选择【语音合成模型】以继续。语音识别模型名称中一般包含“tts”等关键词。目前支持的供应商：openai、硅基流动等。注意！音色需要与供应商的模型匹配，否则无法使用，默认音色列表中仅包含了openai的音色，其他供应商的音色需要手动添加。参考音色暂时只支持硅基流动，其他供应商请不要选择参考音色！',
        'noRefAudio': '不使用参考音频',
...
key3`,
        ...
        'start_read': '开始阅读',
        'stop_read': '停止阅读',
        'longTextPlaceholder': '请输入需要被阅读的长文本，配合桌宠机器人可以实现数字人口播功能。你可以使用<音色名></音色名>将使用对应音色括起来，实现多音色朗读。如果你导入的是epub格式的文件，你可以点击右上角的按钮切换章节。',
        'selectFile': '选择文件',
        ...
        'EnterFirstPerson': '进入第一人称(WASD+QE)',
      ...
      'bodyPlaceholder': `Enter the body in JSON Schema format. For example:
{
    ...
    "required": ["name", "age"]
        ...
        'clickOrDropCharacterCards': 'Click or drag the character card JSON file here',
...
Key3`,
        ...
        'longTextPlaceholder': 'Please enter a long text that needs to be read, with the table pet robot, the digital population broadcast function can be realized.You can use <timbre name> </timbre name> to enclose the corresponding timbre to achieve multi-timbre reading.If you are importing an EPUB file, you can click the button in the upper right corner to switch chapters.',
        ...
        'EnterFirstPerson': 'Enter First Person(WASD+QE)',
